...
import com.littletrips.api.model.dto.Trip;
import com.littletrips.api.model.dto.TripHistory;
import com.littletrips.api.service.db.TransactionLoader;
...
public class TripService {
    ...
    public TripHistory getTripHistoryByPAN(String pan, Integer page, Integer size) {
        ...
        if (startIndex >= userTrips.size() || startIndex < 0) {
            return new TripHistory(new ArrayList<>(), userTrips.size(), page, 0, 0, 0, 0);
        }

        List<Trip> tripSubList = userTrips.subList(startIndex, endIndex);
        return new TripHistory(tripSubList, userTrips.size(), page, tripSubList.size(),
                getMonthSpending(userTrips), getLifetimeSpending(userTrips), getMonthTripCount(userTrips));
    }
